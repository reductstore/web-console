import { hasWritePermissionForBucket, checkWritePermission } from './permissionUtils';
...
describe('hasWritePermissionForBucket', () => {
  ...
  it('should handle exact bucket name matching', () => {
    ...
    expect(hasWritePermissionForBucket('special-bucket', allowedBuckets)).toBe(true);
  ...
  it('should handle wildcard patterns at the end', () => {
    ...
    expect(hasWritePermissionForBucket('bucket-123', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-test', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-a-b-c', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket', allowedBuckets)).toBe(false);
    expect(hasWritePermissionForBucket('other-bucket-123', allowedBuckets)).toBe(false);
  });

  it('should handle wildcard patterns at the beginning', () => {
    const allowedBuckets = ['*-bucket'];
    
    expect(hasWritePermissionForBucket('test-bucket', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('123-bucket', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('-bucket', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('a-b-c-bucket', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket', allowedBuckets)).toBe(false);
    expect(hasWritePermissionForBucket('test-bucket-123', allowedBuckets)).toBe(false);
  });

  it('should handle wildcard patterns in the middle', () => {
    const allowedBuckets = ['bucket-*-data'];
    
    expect(hasWritePermissionForBucket('bucket-test-data', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-123-data', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket--data', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-a-b-c-data', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-test', allowedBuckets)).toBe(false);
    expect(hasWritePermissionForBucket('bucket-data', allowedBuckets)).toBe(false);
    expect(hasWritePermissionForBucket('test-bucket-data', allowedBuckets)).toBe(false);
  });

  it('should handle multiple wildcard patterns', () => {
    const allowedBuckets = ['*-bucket-*'];
    
    expect(hasWritePermissionForBucket('test-bucket-123', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('a-bucket-b', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('-bucket-', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket-123', allowedBuckets)).toBe(false);
    expect(hasWritePermissionForBucket('test-bucket', allowedBuckets)).toBe(false);
  });

  it('should handle mixed exact and wildcard patterns', () => {
    const allowedBuckets = ['exact-bucket', 'wildcard-*', '*-suffix', 'another-exact'];
    
    // Exact matches
    expect(hasWritePermissionForBucket('exact-bucket', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('another-exact', allowedBuckets)).toBe(true);
    
    // Wildcard matches
    expect(hasWritePermissionForBucket('wildcard-123', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('test-suffix', allowedBuckets)).toBe(true);
    
    // Non-matches
    expect(hasWritePermissionForBucket('other-bucket', allowedBuckets)).toBe(false);
  ...
  it('should escape special regex characters in patterns', () => {
    ...
    expect(hasWritePermissionForBucket('bucket.test.123', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucket+special.data', allowedBuckets)).toBe(true);
    expect(hasWritePermissionForBucket('bucketXtest.123', allowedBuckets)).toBe(false);
    expect(hasWritePermissionForBucket('bucketXspecial.data', allowedBuckets)).toBe(false);
  });
...
describe('checkWritePermission', () => {
  ...
  it('should check write permissions with wildcard matching', () => {
    const permissions = { fullAccess: false, write: ['bucket-*', 'test-*-data'] };
