...
import { MODE_SELECT_OPTIONS, MODE_SELECT_STYLE } from "./LifecycleModeUtils";

interface BulkModeChangeModalProps {
  count: number;
  open: boolean;
  onConfirm: (mode: LifecycleMode) => void;
  onCancel: () => void;
  loading?: boolean;
...
export default function BulkModeChangeModal({
  ...
  const [selectedMode, setSelectedMode] = useState<LifecycleMode>(
    LifecycleMode.ENABLED,
  );
  const { token } = theme.useToken();

  const plural = count === 1 ? "lifecycle" : "lifecycles";
  ...
  return (
    <Modal
      ...
      afterOpenChange={(visible) => {
        if (!visible) setSelectedMode(LifecycleMode.ENABLED);
      }}
    >
